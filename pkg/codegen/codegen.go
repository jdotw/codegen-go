...
func constructImportMapping(swagger *openapi3.T, projectName string, input map[string]string) importMap {
	...
	for _, s := range tags {
		result["/internal/app/"+s] = goImport{Name: strings.ToLower(s) + "app", Path: "github.com/12kmps/baas-" + projectName + "/internal/app/" + strings.ToLower(s)}
		result["types-/pkg/"+s] = goImport{Name: strings.ToLower(s) + "api", Path: "github.com/12kmps/baas-" + projectName + "/pkg/" + strings.ToLower(s)}
	}
...
func GenerateTypesForSchemas(t *template.Template, schemas map[string]*openapi3.SchemaRef, excludeSchemas []string) ([]TypeDefinition, error) {
	...
	for _, schemaName := range SortedSchemaKeys(schemas) {
		...
		types = append(types, goSchema.NestedTypes...)
	...
	for _, k := range SortedPathsKeys(paths) {
		...
		for _, o := range path.Operations() {
			if o.RequestBody != nil && o.RequestBody.Value != nil {
				b := o.RequestBody.Value.Content.Get("application/json")
				if b == nil {
					b = o.RequestBody.Value.Content.Get("application/json-patch+json")
				}
				if b != nil {
					s := b.Schema
